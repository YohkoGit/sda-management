...
## Departments — configurable, design-safe

The number of departments is admin-configurable (the original BMAD spec named six, but a real church may have 10, 15, or more). The visual system handles this with a **two-part identity**:

1. **Abbreviation** (DIACONNAT, JA, MIFEM…) — set in IBM Plex Mono, uppercase, 0.12em tracking. This is the **primary** identifier; the design relies on it for disambiguation everywhere a department is referenced.
2. **Spot color** — secondary, decorative. Admin chooses from a curated palette of **24 tokens**, all at OKLCH `L 0.45 C 0.07` with hue varying in 15° steps (plus 4 alternates at slightly darker L 0.40 for variety).

### Why constrained instead of free picker

A free color picker produces clashing or muddy palettes the moment a non-designer picks something. By constraining admins to 24 pre-approved spot colors with matched chroma and lightness, every department feels like it belongs to the same family — no matter how many exist.

If the church ever exceeds 24 departments, colors **recycle**. The abbreviation still disambiguates. The system does not break visually.

### Data model

```ts
type Department = {
  code: string;          // "DIACONNAT", "JA", "MIFEM" — uppercased abbreviation
  name: string;          // "Diaconat" — display name
  colorToken: number;    // 1..24 — index into the dept palette
  // ...other fields (parent, members, etc.)
};
```

**Storing the token index (not a hex)** lets future palette adjustments (e.g. tuning the chroma/lightness band) cascade to every department automatically without a data migration.

In CSS, reference the color as `var(--dept-${pad2(colorToken)})` — for example token 7 → `var(--dept-07)`. In Tailwind, generate utilities like `bg-dept-07`, `text-dept-07`, etc. via a small theme extension.

### Admin UX — Department create/edit

The department create/edit form includes a **swatch picker**:
- A grid of 24 circular swatches (12 columns × 2 rows)
- Already-used swatches show greyed out (35% opacity) with a hairline border — pickable but discouraged
- Selected swatch shows a 2px ink ring with a 1px parchment gap and a gilt outer glow
- Default selection is the next unused token (so admins rarely need to pick)
- Caption below the grid: `● Sélectionnée · token 07 · ○ Déjà utilisée · réutilisable`

This is rendered live in the prototype's design system tab (section 01). See `prototype/design-system.jsx` for the swatch-picker pattern; reuse it inside the existing `AdminDepartmentsPage.tsx` form.

### Where the color shows up

| Surface | Treatment |
|---|---|
| Calendar event chips | 4px dot in the dept color, followed by event title |
| Dept tag in lists | Mono caps code in `var(--ink-2)` on a `var(--parchment)` chip with a hairline border — **color is the dot beside it**, not the chip background |
| Sub-ministry headers | Small swatch dot + name. Color decorative only. |
| Activity assignment chips | Avatar tinted with the assignee's primary department color |
| Contact picker section headers | Swatch + dept eyebrow + count |

Never use the dept color as a large fill (button, header background). It works as a **dot, a 2px stripe, or an avatar tint** — anything bigger and clashes start showing between adjacent department colors.
