...
public class InitiateAuthRequestValidatorTests
{
    private readonly InitiateAuthRequestValidator _validator = new();

    [Fact]
    public async Task ValidEmail_ShouldPassValidation()
    {
        var request = new InitiateAuthRequest { Email = "user@example.com" };
    ...
    {
        var request = new InitiateAuthRequest { Email = email! };
        var result = await _validator.TestValidateAsync(request);
    ...
    public async Task EmailExceeding255Chars_ShouldFailValidation()
    {
        ...
        var request = new InitiateAuthRequest { Email = longEmail };
        var result = await _validator.TestValidateAsync(request);
        // Exactly 255 should pass
        result.ShouldNotHaveValidationErrorFor(x => x.Email);

        var tooLongEmail = new string('a', 245) + "@test.local"; // 256 chars
        var request2 = new InitiateAuthRequest { Email = tooLongEmail };
    ...
    {
        var request = new InitiateAuthRequest { Email = "user\0@example.com" };
        var result = await _validator.TestValidateAsync(request);
        result.ShouldHaveValidationErrorFor(x => x.Email);
    }
}
